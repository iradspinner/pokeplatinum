...
What is left out: critical hits, and anything the real attack applies after this point in its battle script, such as Life Orb and the damage-halving berries. The engine's variable-power moves that set their power in a battle script (Weather Ball, Brine, Payback and the like) are seen at their listed base power, because the AI only reproduces the handful of cases in the table above.
