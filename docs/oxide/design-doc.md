...
Design document, v0.8 (2026-09-15). This file is written for Claude to work from. Read it in full at the start of every session, then read `Platinum Oxide - Tracker.md`, then act.
...
| Battle AI updates | wanted | Platinum's own trainer AI (in `src/battle/trainer_ai/`) is the baseline, updated to know the new moves and abilities and with the ds-pokemon-hacking battle_edits AI bug fixes applied (Ian applied some already). Hardlove's AI is **not** ported. References Ian uses: lhearachel's Gen 4 AI gist (ff61af1f58c84c96592b0b8184dba096), pokemow.com/Gen4/TrainerAI/ and its switching page |
...
- Which species make the cut. Needed before the species-slot port starts.
- The five **Q**s in `notes\phase3-base-rom-inventory.md` section 2 (which of the base ROM's code patches to keep).
