...
## 6. The three approaches, as they now look (facts only; the considerations breakdown for Ian is Phase 2)

- **A. Data-layer only (DSPRE and scripts, no code changes).** Cannot deliver any of the four scoped elements. Every one of them requires code patches (section 1). This is the ceiling the earlier project note identified, confirmed.
- **B. Patch kit re-targeted to Platinum (the hg-engine / plat-engine model).** Start from plat-engine's skeleton, port hg-engine's C into it, and re-derive every hook, repoint and armips edit against Platinum's binaries using pokeplatinum as the map. Output is a `test.nds` built from Ian's base ROM the way hg-engine builds from vanilla HG. The base ROM's existing DSPRE edits survive as long as they are in NARCs the kit does not regenerate, and are lost for NARCs it does regenerate unless re-exported into the kit's source format first (the hg-engine README says the same about scripting/mapping edits: do them before applying the engine, or do them in the engine's formats).
- **C. Full source build on pokeplatinum.** Modify the decomp's C directly (add Fairy to the chart, widen the ability field, raise the caps, add moves and abilities) and build a new ROM from source. No hooks or address hunting at all, because the source *is* the game. Cost: Ian's existing base-ROM edits have to be re-expressed as changes to pokeplatinum's data files, and hg-engine's C has to be adapted to pokeplatinum's function names and structs rather than dropped in. Note that the earlier project note describing plat-engine as "built on top of pokeplatinum" was wrong: the plat-engine repo is a patch kit in the hg-engine mould and does not use the decomp. Whether anyone has since started a decomp-based Platinum engine was not checked and should be, when the breakdown is written.

## 7. Facts about the base ROM found along the way

- Platinum extraction: 122 overlays (0..121), none compressed (DSPRE decompressed them). `arm9.bin` is 1,056,032 bytes, uncompressed. `arm9.bin` and `ov016.bin` carry a later modification time (2026-08-25) than the rest of the extraction (2026-08-03), and several data NARCs (`pl_personal`, `evo`, `wotbl`, `pl_waza_tbl`, `trdata`, `trpoke`, `pl_poke_icon`, `pl_batt_bg`, `pl_batt_obj`, `height`, `pl_poke_data`, `trtbl`) were rewritten 2026-08-30. So DSPRE has already written into both the code and several data tables of this base. Before any address from plat-engine or pokeplatinum is used, `arm9.bin` and `ov016.bin` must be diffed against a vanilla Rev 1 to see what changed.
- `Platinum Oxide VSMaker2 Data\` inside the DSPRE contents folder is a complete second extraction of the same ROM (arm9, overlays, full `data\` filesystem, its own `unpacked\`), dated 2026-08-04, made by VSMaker2. It is a stale duplicate of everything else in the folder and is not read by DSPRE. Treat it as a trainer-editor artifact only; do not take any table from it as current. Consider moving it out of the DSPRE contents folder to avoid the two-copies problem.
- Hardlove extraction: 152 overlays (0..151), 105 of them compressed; `arm9.bin` 1,120,352 bytes.
