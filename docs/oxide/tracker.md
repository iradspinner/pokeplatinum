...
- [ ] Add optional `ability` (0|1|2) and `gender` (null|"male"|"female") per-mon fields to trainer JSON and to `tools/dataproc/src/trainerproc.c`
- [ ] Make `TrainerData_BuildParty` (`src/trainer_data.c`) honor those fields, implemented correctly (pick a personality that satisfies the requested gender for that species' ratio) rather than copying the base ROM's buggy +/-2 nudge
- [ ] Extend `tools/oxide/import_base_rom.py` with a trainer importer for the 488 changed trainers, decoding `ivScale`'s low byte as `iv_scale`, high nibble as `ability`, low nibble as `gender`
