...
- [ ] **Ian: two questions left over from the pick-list.** (1) Twenty-one native species where `New Pokedex.xlsx` and the base ROM disagree on base stats - rule on which source wins, per row or wholesale. Note the sheet is stale in at least one place (Swinub's row is vanilla while the base ROM's is buffed). (2) Evolution triggers for the two alt-evolution megas, Gyarados M and Lopunny M; their donor slots and stat blocks are settled, the trigger is not
