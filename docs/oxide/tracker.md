...
**In progress:** Phase 3. Species/move/evo/learnset import done, verified (2026-09-15). Five Qs answered and the trainer format corrected to vanilla (`docs/oxide/phase3-answers-and-trainer-format.md`). Trainer ability/gender fields, game logic, and the base-ROM importer are all done (2026-09-15): 775 of 928 trainers carried over, verified field-by-field; the other 153 (bigger custom parties, different size than vanilla) need JSON-array-growth tooling that doesn't exist yet, next up if picked up again. A roadmap for the rest of Phase 3 and all of Phase 4, with file-level detail for the next several items, was written in the session that did this work; ask if it needs to be written down again. Waiting on Ian: WSL2 setup confirmation, the species pick-list, confirmation on the ability/gender nibble-direction guess (1=male/2=female) once it can be checked in an emulator
...
- [x] Carried over 775 of 928 trainers (header + party, including ability/gender); the other 153 have a different party size in the base ROM than vanilla (bigger custom parties) and are logged in `tools/oxide/import_report.md`, not applied - needs a way to insert whole new party-member objects into the JSON array, not built yet
- [x] Verified: `make rom` succeeds. Byte-identical NARC comparison no longer applies (the trainer-mon struct is 2 bytes wider now), so verification decodes both sides field-by-field instead: 0 mismatches across all 775 same-party-size trainers
...
- 153 trainers whose base-ROM party size differs from vanilla (Ian gave them bigger custom parties, e.g. trainer index 5) still need their full party carried over. `tools/oxide/import_base_rom.py` logs these (see `import_report.md`) rather than applying, since growing a JSON array by inserting whole new party-member objects needs a `jsonstyle.py` primitive that doesn't exist yet (only `insert_key`, for adding a key to an existing object, was built)
