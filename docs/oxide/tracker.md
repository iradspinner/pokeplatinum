...
**Waiting on Ian** (the full wording of every entry shortened here is in the archive). Ian runs the in-game checks below all at once when the new CPU is in (2026-09-26):
...
- **The staples engine rulings in game** (2026-09-26, merged the same day): the test kit's Modern rules menu, 18 entries, described in `docs/oxide/test-kit.md`. Defog clears hazards from both sides and screens only from the target's (Ian confirmed, 2026-09-26).
...
- [ ] **4. Move expansion.** Left short on purpose: eight move names clip by a few pixels on the summary screen; contest data for the 452 new moves is a derived default; the 56 Z-moves are inert data; a level-0 evolution move becomes level 1, so an evolved Pokemon gets it only from the Move Relearner
  ...
  - [ ] **Rage Fist** (Ian, 2026-09-26): 50 plus 50 per hit taken, to 350, under the Gen 9 rule that switching out or fainting keeps the count
