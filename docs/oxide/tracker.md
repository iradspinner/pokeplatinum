...
- [ ] Carry over scripts (91), events (158) and their text (60 banks). **Plan: `docs/oxide/phase3-scripts-and-events-plan.md`** (2026-09-20). 184 distinct maps are touched: 65 with both script and event changes, 26 script-only, 93 events-only. The three halves are not separable per map, so the unit of work is a map done across all three at once
  ...
  - [ ] Script disassembler. `scr_seq.narc` is bytecode; the decomp keeps it as macro assembly. Generate the opcode table from `asm/macros/scrcmd.inc`, then prove the disassembler by round-tripping all 1,124 vanilla script files to byte-identical output before touching the base ROM's 91. This is the real cost of the item
