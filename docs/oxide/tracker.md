...
- The EV/IV viewer is unreachable while the button mode is "L=A": `ApplyButtonModeToInput` clears both L and R for that mode before any screen sees them. Vanilla behaviour, inherited by the base ROM too. Rebind the viewer or special-case that mode if it ever matters
...
- Options-menu text still says "Start is X" / "Swap X/Y" for the two button-mode values repurposed as the frame-rate uncap selector; needs relabeling once text-import tooling exists
