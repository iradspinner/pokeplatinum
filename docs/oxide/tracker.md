...
- **Rage Fist** (2026-09-26): keep Hardlove's flat 100, or port the later 50 plus 50 per hit taken?
...
- [ ] **4. Move expansion.** Left short on purpose: eight move names clip by a few pixels on the summary screen; contest data for the 452 new moves is a derived default; the 56 Z-moves are inert data; a level-0 evolution move becomes level 1, so an evolved Pokemon gets it only from the Move Relearner
  ...
  - [ ] **Computed powers left**: Round, the Fusions, the Pledges and terrain's five. Foul Play, Body Press, Psyshock, Sacred Sword, Darkest Lariat, Freeze-Dry and Flying Press (learnable), Psystrike, Secret Sword and Chip Away pick no other stat or type
...
- [ ] **Balance analysis** (Ian, 2026-09-22): its own track, covering the whole game's balance, with its status in `docs/oxide/balance-plan.md`; B5 is its current work, run one suite at a time on one core until the new CPU is in. The vendored calculator's defects and its missing element 5 and staples rules are fixed and merged (9195d1a64); item 22, the calculator following the engine's computed powers, is parked on `encounter-calc-item22` (949defdc4) for the next full rescore. **The three passes below are that track's work**; their status lives in its plan. This entry's wording as of 2026-09-23 is in the archive
...
- [ ] **Open bug (2026-09-26): Transform copies only the ability's low byte** since element 2 widened it, so a target ability id of 256 or more comes out wrong
