...
3. **Cloud jobs**, one branch each, reviewed and merged by the Overseer: the staples survey's engine rulings are done on `cloud/element5-staples` and wait on review and the base-ROM checks (Phase 5).
...
- **Ian's second gift pass, in game** (2026-09-26, merged in 2c48803ab): Riley's egg on Iron Island hatches as a random species (one of eight lines); Mindy in Snowpoint takes a Snover and gives a Suicune, which should be shiny; the Drifloon balloon no longer appears at Valley Windworks; and the Hearthome Fan Club member only says goodbye, with no starter gift.

- **Saturn 2's permanent Trick Room, in game** (2026-09-26, 7657103c9, merged the same day): fight Saturn at Galactic HQ. The battle should open with "The dimensions became distorted!", the slower Pokemon should move first all fight (no "The twisted dimensions returned to normal!" after five turns), a Trick Room from either side should fail, and Saturn's Uxie should not choose it.

- **Swarms, radar and GBA encounters off, in game** (2026-09-26, merged with 7cefbba53; the finished entry is in the archive), on a save with the National Dex and the game beaten: Acuity Lakefront's grass and Poke Radar give no Weavile, Abomasnow, Mamoswine or Glalie and chains still build; the Sandgem sibling keeps her ordinary line; Sinnoh Now shows no swarm flash.
- **Native moves at modern numbers, in game** (2026-09-26, `cloud/main-native-moves`; its last commit lists every change, and Ian's three rulings on it are in). Fury Cutter used five times in a row should go 40, 80, 160, 160, 160 in power (the damage should stop growing after the third hit), and Roar and Whirlwind should never miss
...
- **The staples engine rulings in game** (2026-09-26, merged the same day): the test kit's Modern rules menu, 18 entries, described in `docs/oxide/test-kit.md`. **A question:** Defog now clears hazards from both sides but screens only from the target's, as the later games and the survey's answer 7 have it, where the job's brief said screens from both; should the user's own screens go too?
- **Element 5 in game.** The test kit's abilities menu (ROM from `tools/oxide/fetch-rom --testkit`, merged on 2026-09-26) has one entry per ability that shows itself, Neutralizing Gas last (merged 2026-09-26); `docs/oxide/test-kit.md` says what to look for. Healer, Hospitality, Telepathy, Battery, Soul Heart's ally case and Flower Veil's partner case need a double battle, and Magician a foe holding an item, which the kit cannot set up
- **A decision for element 4: port terrain, or not** (2026-09-22). Element 5's Electric, Grassy, Misty and Psychic Surge and Seed Sower wait on it too. Terrain is a whole field mechanic Platinum lacks, set by the four Terrain moves, ended by Steel Roller and Ice Spinner, and read by six moves, the Surge abilities and the terrain seeds; hg-engine's C for it spreads over 15 files. **Without it**, the four terrain moves say "But nothing happened!" and the rest are plain hits, though Steel Roller would faithfully always fail. Nothing else waits on this
- **Element 4 in game.** The test kit (`docs/oxide/test-kit.md`, ROM from `tools/oxide/fetch-rom --testkit`) passed sets 1 to 22 in rounds 1 to 3. Still unseen: sets 23 to 26, sets 27 to 31, merged on 2026-09-25 (Sticky Web, After You, Aurora Veil, the four side guards against the kit's two new wild foes, and Belch with the Sitrus Berry the set gives), and Pollen Puff's ally heal, Coaching and Incinerate burning a berry, which the kit cannot set up. At the Move Relearner, check that no name clips beyond the eight known ones, that descriptions read sensibly in five lines and that a new species' moves are listed. **A stub effect does its damage and skips its extra, or says "But nothing happened!"; that is expected.** Autotomize prints no "became nimble!", as in hg-engine, and Ian ruled to keep it that way
...
- [ ] Still not ported from the base ROM's arm9, each needing real understanding: the Vs. Seeker as a portable PC (`UseVsSeekerFromMenu`, two sites; its item text is already in), without the base ROM's move relearner in it (Ian, 2026-09-26), **which Ian ruled on 2026-09-26 should also give infinite Rare Candies** (B4 found about 30 before the League against the 250 to 440 a team of six needs), the palette hue-shift patch (six hooks, low priority, droppable), and an unidentified two-byte change to a lookup table at arm9 `0x0EC478`. The Battle Arcade commands are closed
...
- [ ] **4. Move expansion.** Left short on purpose: eight move names clip by a few pixels on the summary screen; contest data for the 452 new moves is a derived default; the 56 Z-moves are inert data; a level-0 evolution move becomes level 1, so an evolved Pokemon gets it only from the Move Relearner
  - [ ] **The new battle effect scripts.** `tools/oxide/convert_battle_scripts.py` converts hg-engine's (cloned at `~/hg-engine`); `--audit` lists what is left. An effect the audit calls unreachable, because no Oxide species learns a move using it, is not ported (Ian, 2026-09-22) and comes back if the TM or tutor pass makes one of its moves learnable
    - The focused pass (2026-09-25, merged in 7b6ece777) is in the archive.
  ...
  - [ ] **Electro Ball has no power code** (found by the encounter track, 2026-09-26): the game hits with power 1, and the calculator copies that until the engine computes it from the Speed ratio
...
- [ ] **5. Ability effects.** Reimplement the new abilities' behaviour in the decomp's battle code from hg-engine's `ability.c` and `individual/*.c`
  - The pass itself (2026-09-26, 52 of the 59 abilities and Entrainment) and Neutralizing Gas (0eb1b2e9a) are merged and in the archive; its report is the last commit on `cloud/element5-abilities`.
  - [ ] **Not done.** The four Surge abilities and Seed Sower wait on the terrain decision; Ian ruled on 2026-09-26: **Symbiosis is not ported** and does nothing; and **Sharpness keeps hg-engine's longer slicing list** with the five claw moves. Infiltrator passes screens and Mist but not Substitute or Safeguard. Pastel Veil does not cure its partner on switch-in. Unnerve is checked where Berries are used in `battle_lib.c` and for Custap; the type-resist Berries' halving was not traced, so it may not stop those
  - [ ] **Known limits, kept on purpose or small**: Soul Heart raises only the fastest holder; Mirror Armor reflects without a message; Cursed Body uses Disable's message; the ability stat changes of batch 5 skip Contrary and Mirror Armor; Protean takes the move's type before its script runs (Hidden Power and Weather Ball count as Normal); Pixilate and Liquid Voice change only moves with power; Magician does not run on a hit that set off the target's own on-hit ability
- [ ] **6. Battle AI.** Platinum's own AI is the baseline; Hardlove's is not ported. `docs/oxide/battle-ai/` (README first) is the write-up and the table of fixes; the fixes so far are archived.
  ...
  - [ ] Teach it everything Phase 4 changed (2026-09-26 list): the new moves and abilities, Neutralizing Gas, the held items that change a decision (Eviolite, Assault Vest, Air Balloon, Rocky Helmet, Weakness Policy, the seeds), the staples rules (the stale spots are in `cloud/element5-staples`'s report: Simple, the trapping copies in `trainer_ai.c`, Illuminate, 2x critical hits), and Saturn 2's permanent Trick Room, since it guesses who moves first from raw Speed in places
...
- [ ] **The Battle Zone before the Elite Four** (Ian, 2026-09-25; `docs/oxide/battle-zone-plan.md`). Since 2026-09-26 the zone opens **directly after Galactic HQ**, splitting the Galactic fights in two (Ian's targets: HQ hard, the zone medium hard, the climb and the last Galactic fights very hard): **HQ, cap 60** (the Warehouse and HQ, closing on Cyrus 2 and Saturn 2), then **Galactic, cap 65** (the zone, the Coronet climb, Spear Pillar and the Distortion World, closing on Cyrus 3); Candice 56, Volkner 68 and League 78 unchanged. The balance and encounter tools have the splits. Cyrus 2 and Spear Pillar's Mars and Jupiter need roster work in the trainer pass. **Heatran becomes a draw from the legendary pool**, and the Battleground rematches are skipped for now. Earlier wordings are in the archive.
...
- [ ] **Saturn 2 fights under permanent Trick Room** (Ian, 2026-09-26): the Galactic HQ fight (`TRAINER_COMMANDER_SATURN_GALACTIC_HQ`) opens with Trick Room that lasts the whole battle. Ian's rulings: a Trick Room used by either side during the fight **fails**, so the room cannot be cancelled; the opening message is **the standard one**; and it is **Saturn 2 only** for now. His Saturn 2 team is already built for it (`balance-plan.md`).
  ...
  - [ ] Trainer pass, two changes to Saturn 2's team (Ian, 2026-09-26): Uxie's Trick Room, which now always fails, is swapped for another move, and Rhyperior's Choice Scarf, which under the permanent room only makes it move later, is swapped for another item. The replacements are the trainer pass's to choose
- [ ] **The staples survey's rulings** (Ian, 2026-09-26; the survey's "Ian's answers" has all ten):
  - The native move pass (84 fields over 68 moves, with Ian's follow-ups) merged on 2026-09-26 (62bc4f3fb); its sub-item is in the archive. The balance track rescores
  - The engine sub-item (2026-09-26, `cloud/element5-staples`) is in the archive; its report is the last commit on that branch.
  - [ ] **The player never controls weather**, apart from the game's one Ability Patch and Defog on fog: weather moves leave every player learnset, TM and tutor, and weather abilities leave the regular slots of every obtainable Pokemon. The TM and ability passes carry it; trainers keep theirs
  - [ ] Design pass 3: nine species' modern stat buffs, Chimecho and Staraptor raised, Cresselia uncut; the ability pass weighs Magic Guard on the Abra line and trainer-only Drizzle Pelipper and Drought Torkoal
  - [ ] Field moves work on their badge alone (element 8)
- [ ] **The legendary pool's draws, scripted** (main track; the encounter plan's decision 8): nothing in the game rolls them yet. Per `docs/oxide/encounters/availability-plan.json`'s `pool.thirds`, Acuity Cavern draws one of Articuno, Cresselia or Pheromosa (Ian, 2026-09-26), Valor Cavern is off, and Mesprit's roamer draws from its own third; Heatran's draw is the Battle Zone entry's sub-item. Until then the encounter linter's R12 reports these lines.
- [ ] **Balance analysis** (Ian, 2026-09-22): its own track, covering the whole game's balance, with its status in `docs/oxide/balance-plan.md`; B5 is its current work, run one suite at a time on one core until the new CPU is in. The vendored calculator's defects and its missing element 5 and staples rules are fixed on the encounter branch (9195d1a64), merging with the next rescore. **The three passes below are that track's work**; their status lives in its plan. This entry's wording as of 2026-09-23 is in the archive
