...
**Where things stand (2026-09-26).** Phase 3 is finished. **Phase 4 is past elements 4 and 5**: every reachable move effect and 53 of the 59 new abilities are in, the rest waiting on Ian's terrain decision; element 3 has only fill-in left, and element 6, the AI, runs ahead of order. Cloud sessions do the heavy engine work on their own branches (CLAUDE.md, Cloud sessions). **The CPU is degraded until its replacement arrives**, so Ian's playtest ROMs come from `tools/oxide/fetch-rom`. **Bug fixing** lives in the "Open bug" entries under Phase 5, worked with the `debug-live` skill. `git log -1` is the resume point.
...
3. **Cloud jobs**, one branch each, reviewed and merged by the Overseer: the engine side of turning off swarm, radar and GBA encounters, Neutralizing Gas, then the staples survey's engine rulings (Phase 5).
...
- **Element 5 in game.** The test kit's abilities menu (ROM from `tools/oxide/fetch-rom --testkit`, merged on 2026-09-26) has one entry per ability that shows itself, Neutralizing Gas last once its branch merges; `docs/oxide/test-kit.md` says what to look for. Healer, Hospitality, Telepathy, Battery, Soul Heart's ally case and Flower Veil's partner case need a double battle, and Magician a foe holding an item, which the kit cannot set up
...
- [ ] **5. Ability effects.** Reimplement the new abilities' behaviour in the decomp's battle code from hg-engine's `ability.c` and `individual/*.c`
  - The pass itself (2026-09-26, 52 of the 59 abilities and Entrainment, merged the same day) is in the archive; its report is the last commit on `cloud/element5-abilities`.
  ...
  - [ ] **Neutralizing Gas**: on `cloud/element5-neutralizing-gas` to merge, its last commit the report. The trainer AI does not know about the gas
...
- [ ] **Saturn 2 fights under permanent Trick Room** (Ian, 2026-09-26): the Galactic HQ fight (`TRAINER_COMMANDER_SATURN_GALACTIC_HQ`) opens with Trick Room that lasts the whole battle. Ian's rulings: a Trick Room used by either side during the fight **fails**, so the room cannot be cancelled; the opening message is **the standard one**; and it is **Saturn 2 only** for now. His Saturn 2 team is already built for it (`balance-plan.md`).
  ...
  - [ ] Element 6, later: Platinum's AI guesses who moves first from raw Speed in places, so under a permanent room Saturn's AI may misjudge a few choices; teach it the room when element 6 next runs
- [ ] **The staples survey's rulings** (Ian, 2026-09-26; the survey's "Ian's answers" has all ten):
  - [ ] Native moves to the full modern set, keeping the base ROM's deliberate values and Gen 4 accuracy on Thunder Wave, Dark Void and Swagger. Data only; the balance track rescores after
  - [ ] Engine, after element 5: modern behaviour for every native ability, the Gen 6 type immunities, 1.5x critical hits at modern rates, Defog clearing both sides (and still fog), Rapid Spin's Speed raise; then element 6 teaches the AI
...
- [ ] **Balance analysis** (Ian, 2026-09-22): its own track, covering the whole game's balance, with its status in `docs/oxide/balance-plan.md`; B5 and the B3b reference pass are its current work, run one suite at a time on one core until the new CPU is in. Three calculator defects are for the encounter track, which owns the vendored calculator: it applies a dual type's two factors in the opposite order to the game (Crunch into Bronzor reads 42 to 50, the game 43 to 51), and its Generation 4 code gives no damage for Electro Ball, Heavy Slam, Psywave, Super Fang and Trump Card. It also knows none of element 5's 52 new abilities, so no score counts them; Ian gave that to the encounter track too (2026-09-26), to close before the trainer pass. **The three passes below are that track's work**; their status lives in its plan. This entry's wording as of 2026-09-23 is in the archive
