...
- [ ] EV/IV viewer on the summary screen (hg-engine: `IMPLEMENT_NEW_EV_IV_VIEWER`, hooks `Summary_IVEV`/`Summary_Entry_Hook` at `armhooks/`; pokeplatinum's summary screen is `src/applications/pokemon_summary_screen/`, not yet mapped to this). Scope decided (2026-09-15): numbers-only, no relabeled text (avoids the text-bank dependency)
