...
@ Byte-exact but machine-shaped (except that every movement block is now 4-aligned with .balign,
@ as vanilla does: the SetRepelSteps rewrite shortened the file by 15 bytes and shifted every block
@ behind it to an odd offset, and the ARM9 drops the low bit on halfword loads, so the whiteout
